...
使用Eureka应用服务发现、配置管理等功能，您需要购买相应的资源进行搭建和维护，耗时耗力。使用MSE构建的Eureka引擎，您仅需关注Eureka的构建位置、版本、网络和规格，不必关注Eureka构建和维护，更加聚焦业务本身的实现。本文介绍如何在MSE上为应用创建Eureka引擎。

已创建专有网络，并确保网络可用。相关操作，请参见[创建专有网络](/cn.zh-CN/专有网络和交换机/管理专有网络/创建专有网络.md)。
